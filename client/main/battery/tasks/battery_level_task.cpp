...
namespace client {
    battery_level_task_t::battery_level_task_t(system_bus_t& bus) : m_bus(bus) {
    }

    battery_level_task_t::~battery_level_task_t() {
        if (m_handle != nullptr) {
            vTaskDelete(m_handle);
        }            
    }

    void battery_level_task_t::init() {
        if (m_handle != nullptr) {
            return;
        }
    ...
    void battery_level_task_t::run_wrapper(void* paramaters) {
        auto* self = static_cast<battery_level_task_t*>(paramaters);
        self->run();
    }

    void battery_level_task_t::run() {
        setup_adc();
    ...
    float battery_level_task_t::get_percentage(int voltage) const {
    ...
    int battery_level_task_t::get_adc_millivolts() {
